    ...
    async function startGame() {
      ...
      class OceanScene extends Phaser.Scene {
        ...
        create() {
          ...
          this.add
            .text(30, 70, `🏝️ 섬 탐험 / 물고기 추적 / 지역: ${regionId}`, {
              fontSize: "18px",
              color: "#cbd5e1",
        ...
        addIsland(x: number, y: number) {
          this.add.circle(x, y, 135, 0xfacc15, 0.8);
          this.add.circle(x, y, 95, 0x65a30d, 1);
          this.add.circle(x, y, 55, 0x16a34a, 1);
        }
        ...
        spawnFish(texture: string, amount: number) {
          for (let i = 0; i < amount; i++) {
            ...
            fish.setScale(Phaser.Math.FloatBetween(0.45, 0.8));
            fish.setAlpha(Phaser.Math.FloatBetween(0.5, 0.9));
        ...
        update() {
          ...
          this.boat.setVelocity(this.move.x * speed, this.move.y * speed);

          if (this.move.x !== 0 || this.move.y !== 0) {
            this.boat.rotation =
              Math.atan2(this.move.y, this.move.x) + Math.PI / 2;
          }
          ...
          for (const fish of this.fishes) {
            ...
            if (dist < 180) {
              ...
              if (fish.body) {
                this.physics.velocityFromRotation(angle, 160, fish.body.velocity);
              }
            } else {
