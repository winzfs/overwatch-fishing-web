...
export class Boat {
  ...
  update(axis: Vector2, boosting: boolean, speedMultiplier: number, dt: number) {
    ...
    const frame = Math.floor(this.animationTime * (moving ? 8 : 3)) % 2;
    this.assetId = moving ? (frame === 0 ? "boat.move.1" : "boat.move.2") : frame === 0 ? "boat.idle.1" : "boat.idle.2";
  }
}
